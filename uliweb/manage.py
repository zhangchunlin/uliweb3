        ...
        for appname in apps:
            ...
            if os.path.exists(path):
                ...
                while ans not in ('y', 'n'):
                    ans = input('The app directory has been existed, do you want to overwrite it?(y/n)[n]')
                    if not ans:
                        ans = 'n'
            else:
                ans = 'y'
...
class MakePkgCommand(Command):
    ...
    def handle(self, options, global_options, *args):
        if not args:
            while not args:
                args = input('Please enter python package name:')
            args = [args]
    ...
    def handle(self, options, global_options, *args):
        ...
        if os.path.exists(project_name):
            if global_options.yes:
                ans = 'y'
            while ans not in ('y', 'n'):
                ans = input('The project directory has been existed, do you want to overwrite it?(y/n)[n]')
                if not ans:
                    ans = 'n'
        else:
            ans = 'y'
    ...
    def handle(self, options, global_options, *args):
        ...
        if os.path.exists(module_name):
            if global_options.yes:
                ans = 'y'
            while ans not in ('y', 'n'):
                ans = input('The module directory has been existed, do you want to overwrite it?(y/n)[n]')
                if not ans:
                    ans = 'n'
        else:
            ans = 'y'
...
class SupportCommand(Command):
    ...
    def handle(self, options, global_options, *args):
        ...
        while not support_type in _types and support_type != 'quit':
            ...
            print('    ' + '\n    '.join(sorted(_types)))
            print()
            support_type = input('Please enter support type[quit to exit]:')
        ...
        for _type, app, m in get_module(command, apps):
            ...
            if mod:
                if hasattr(mod, 'call'):
                    ...
                    print("Can't find call() or main() function in module {}".format(mod_name))
                    r = -1
                exe_flag = True
...
class InstallCommand(Command):
    ...
    def handle(self, options, global_options, *args):
        ...
        for r_file in get_requirements():
            if global_options.verbose:
                print("Processing... {}".format(r_file))
...
class RunserverCommand(Command):
    ...
    def run_asgi(self, options, extra_files, get_app):
        ...
        if server == 'uvicorn':
            ...
            if options.reload:
                cmd.append('--reload')
                # 添加需要监视的目录：三个来源并进同一集合做字符串去重（不做最小根化）
                # 来源1: 项目目录；来源2: apps 目录；来源3: extra_files 所在目录
                project_dir = global_options.project or os.getcwd()
                watched_dirs = set()
                for d in (project_dir, old_apps_dir):
                    if d:
                        watched_dirs.add(os.path.abspath(d))
                for f in extra_files:
                    dir_path = os.path.dirname(f)
                    if dir_path:
                        watched_dirs.add(os.path.abspath(dir_path))
        ...
        if options.reload:
            log.info(' * 自动重载已启用')
            if global_options.verbose:
                log.info(f' * 监视目录: {sorted(watched_dirs)}')
...
class StaticizeCommand(RunserverCommand):
    ...
    def handle(self, options, global_options, *args):
        ...
        print("Export static files to {}".format(dst_path))
        call('uliweb exportstatic {}/static'.format(dst_path))
...
class FindCommand(Command):
    ...
    def _find_option(self, global_options, option):
        ...
        print('------ Combined value of [{}] ------'.format(option))
        print(settings.get_var(option))

        print('------ Detail   value of [{}] ------'.format(option))
