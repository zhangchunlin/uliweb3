...
- [x] ASGI 接口实现（包含同步适配器机制）
  ...
  - 验证：`python -c "from uliweb.core.starlette import AsyncDispatcher; print(AsyncDispatcher)"`
...
2. **代码结构**
   - `uliweb/core/starlette.py`：主要 ASGI 框架（核心）
   - `uliweb/core/SimpleFrame.py`：保留但不再支持 WSGI 接口
