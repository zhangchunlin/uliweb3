...
^- [x] **1. feat(SimpleFrame)：启动期 legacy process_* 告警**
...
^- [x] **2. docs(skill)：references/asgi.md 加"从 process_* 迁移到 dispatch"小节**
...
^- [x] **3. docs(AGENTS.md)：记录 process_* 废弃约定**
...
^- [x] **4. 终检**：仓库根 `nosetests --with-doc test` 全绿（OK）；`git status` 工作区干净；三处改动（SimpleFrame 告警 + skill + AGENTS.md）全部落位。
