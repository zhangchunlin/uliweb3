...
* [Starlette](https://www.starlette.io/) 用它来进行框架的核心处理，比如：
    ASGI 支持、Request、Response、WebSocket 等。
...
* URL处理
    ...
    * 灵活强大的URL映射。采用Starlette的Routing模块，可以非常方便地定义URL，并与View函数
...
* 部署
    ...
    * 支持ASGI服务器部署（如 Uvicorn、Hypercorn、Daphne）
    * 支持WebSocket
    * 支持HTTP/2
    * 可以部署到各种云平台
