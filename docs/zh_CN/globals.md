...
而这里的Request类是基于Starlette的Request来派生的，支持异步访问方式：

{% alert class=info %}
Uliweb3 使用 Starlette 的 Request 类替代了原来的 Werkzeug Request。为了保持兼容性，
提供了 GET, params 等属性。同时，POST, FILES, json 等需要异步访问，请使用
await request.get_POST(), await request.get_FILES(), await request.get_json() 方法。
{% endalert %}

### Response

它也对Starlette提供的Response进行了扩展：

{% alert class=info %}
添加了一个write方法，可以直接使用 response.write() 方法，会更方便一些。
{% endalert %}
...
* `request.is_xhr` 用于检查是否为 AJAX 请求
* `request.get_json()` 异步获取 JSON 数据
* `request.get_POST()` 异步获取 POST 表单数据
* `request.get_FILES()` 异步获取上传文件
* `request.get_params()` 异步获取合并参数（GET + POST）
