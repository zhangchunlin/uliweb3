...
Start a new ASGI development server.

Options:
  -h HOSTNAME, --hostname HOSTNAME
                        绑定IP，缺省为localhost，如果希望让别人访问，可以绑定 0.0.0.0（默认：localhost）
  -p PORT, --port PORT  绑定的端口号，缺省为8000（默认：8000）
  --no-reload           禁止服务器自动重启。缺省是自动重启
  --no-debug            禁止服务器进入调试模式。缺省是进入
  --nocolor             禁用彩色日志输出，缺省为启用
  --asgi-server {uvicorn,hypercorn,daphne}
                        使用的ASGI服务器（默认：uvicorn）
  --asgi-workers ASGI_WORKERS
                        ASGI工作进程数（默认：1）
  --ssl                 使用SSL（需要配置SSL证书）
  --ssl-key SSL_KEY     SSL私钥文件路径
  --ssl-cert SSL_CERT   SSL证书文件路径
  --coverage            使用coverage启动服务器
  --trace-print         跟踪print语句输出
...
{% alert class=info %}
Uliweb3 使用 ASGI 架构，支持以下服务器：
- **uvicorn**（默认）：高性能 ASGI 服务器，基于 uvloop
- **hypercorn**：支持 HTTP/2
- **daphne**：Django 官方推荐
...
```
uliweb runserver                                    # 启动缺省服务器（uvicorn）
uliweb runserver -h 0.0.0.0 -p 8000                 # 允许外部访问
uliweb runserver --asgi-server hypercorn            # 使用 hypercorn 服务器
uliweb runserver --asgi-workers 4                   # 启动4个工作进程
```
