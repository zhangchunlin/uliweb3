...
- [ ] **1. fix(SimpleFrame)：`_init_routes` 改为 URL 级去重/覆盖（一个 endpoint 可多个 URL）+ 单测**
  在 `_init_routes` 内：
...
- [ ] **2. 终检**：仓库根 `nosetests --with-doc test` 全绿（OK）；`git status` 工作区干净；归档 `intent/exposes-multi-url/REVIEW.md`。
