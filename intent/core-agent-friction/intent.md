...
- **是否允许拆分 SimpleFrame.py** → 已定案：**不拆文件**，仅文档化职责边界 + 在文件内分节标注（每个类/功能区加清晰 section 注释），把拆分留给后续独立评估。
- **命名去歧义的范围** → 已定案：**仅文档化**（注释、架构地图、AGENTS.md 说明），**不实际重命名** `url_map`/`router` 等符号（改动面大、风险高，重命名单独评估）。
- **架构地图放哪** → 已定案：**放 AGENTS.md**（在 Architecture 一节内给出模块职责地图与隐式行为规则清单，作为 agent 单一入口；细节文档可链接 docs/zh_CN）。
